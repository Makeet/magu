...
export default function SearchComponent({map}) {
  ...
  //마커의 옵션을 설정해주는 함수
    ...
    //검색 기록 클릭시
    const onClickLog = (log) => {
      // handleSearchPOI가 작동한 후에 setSearch(log)가 된다!!! 왜지!!
      // 수정해야함
      setSearch(log);
      document.getElementById("search_box_input").value = log;
      
      handleSearchPOI();
    }

  //검색어로 주소 검색
  function handleSearchPOI(e){
      ...
      <div>
        <div id="search_box">
          ...
          <form onSubmit={e => handleSearchPOI(e)}>
            <input type="text" id="search_box_input" placeholder="지도 검색" onChange={onChangeSearch}  />
